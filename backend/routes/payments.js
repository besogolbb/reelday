...
//
// To remove after the smoke test:
...
export default async function paymentRoutes(fastify) {
  ...
  fastify.post('/payments/create', { preHandler: fastify.authenticate }, async (request, reply) => {
    ...
    // tier price so the smoke test costs ~₱50 per round-trip instead of
    // ₱1,490 / ₱2,990. Label is tagged [TEST] so the PayMongo receipt
    // makes the override obvious in audit logs.
