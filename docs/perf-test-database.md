...
| Upload kickoff 1000c × 1000 | **379 rps · p95 2.26 s · p99 2.33 s · 0 fail** | **2026-05-24** | **local** |
| Reactions 200c × 1000 | 596 rps · p95 689 ms · 0 fail | 2026-05-16 | local |
| Mixed peak (600u + 1500r + walls / 30s) | **rx p95 7 ms · wall p95 735 ms · upload p95 1.08 s · 0 fail** | **2026-05-24** | **local** |
| Sustained 3-min mix | **0 fail · flat 6/6 · upload/rx p95 5-7 ms · wall p95 17 ms** | **2026-05-24** | **local** |
| Multi-wall + 150 spammers | **0 fail · uploads-poll p95 33 ms · rx-poll p95 19 ms** | **2026-05-24** | **local** |
| Cross-event isolation (200 spammers, 30s) | **Wall B p95 30 ms · 0 fail** | **2026-05-24** | **local** |
...
| Guest-arrival storm (cold first-load) | **500 simultaneous, 0 fail** · p50 time-to-wall 1.18 s @100c | 2026-05-29 | remote |
| Multi-event (N hot at once) | **4 events / 160 guests · 0 server 5xx · wall p95 959 ms** | 2026-05-29 | remote |
...
| 45 | perf:full suite (backend-truth) | `npm run perf:full-local` | local | health 94 ms (server_ms 2) · upload 1000c **379 rps · p95 2263 ms · p99 2330 ms · max 2349 ms** 0 fail · mixed 0 fail (up p95 1080 / **rx p95 7** / wall p95 735 n=10) · sustained 3 min **0 fail, flat 6/6** (**up p95 6 / rx p95 6 / wall p95 17**) · multi-wall **0 fail/4791** (uploads-poll p95 33 / **rx-poll p95 19** / spam p95 4) · cross-event **Wall B p95 30 ms** / Wall A 33 ms 0 fail · **5592 spam writes 0 fail** | ✅ | **Backend-truth re-run of entry 44 from the Easypanel terminal (no laptop pipe).** Definitively closes the entry-44 question: the elevation there was 100% single-laptop network pressure, not backend regression. **SIX new bests in one run.** Upload 1000c p99 **2330 ms** = first sub-3s 1000c ever (beats entry 36's 3417 by 32%). Sustained 3-min **up/rx p95 5-7 ms** = ~14× faster than entry 32's 85/86 (the previous "new steady-state baseline"). Multi-wall uploads-poll **33 ms** = 3.5× faster than entry 43's 114 (previous best). Cross-event Wall B **30 ms** = 5.5× faster than entry 30's 168 (previous best). Mixed reaction **p95 7 ms** with the SDE beacon stamping upload_id on every write (matches/beats entry 35's 11 ms). ~14,000 requests across all 6 sub-tests, 0 failures, 0 rate-limited writes. Conclusively validates that everything shipped since entry 43 — SDE Lambda migration, sdePreflight, sde-lambda-poller, videoTranscode compressed_key persist, admin.ics column drop, photo-message batch fallback — is fully off the request hot path. **Healthiest run on record.** |

### 2026-05-25 — Re-validation run (Tala UI gating + customMusic frontend fix; wall.html reverted to yesterday's state)

| # | Test | Cmd | Env | Result | Verdict | Note |
|---|---|---|---|---|---|---|
| 46 | perf:full suite | `npm run perf:full` | remote | health 790 ms (server_ms 1, cold TLS) · upload 1000c **264 rps · p95 3634 ms · p99 3644 ms · max 3675 ms** 0 fail · mixed 0 fail (up p95 1831 / rx p95 113 / wall p95 1814 n=9) · sustained 3 min **0 fail, flat 6/6** (up p95 116 / rx p95 114 / wall p95 275) · multi-wall **0 fail/3030** (uploads-poll p95 168 / rx-poll p95 218 / spam p95 112) · cross-event **Wall B p95 207 ms** / Wall A 213 ms 0 fail · **3099 spam writes 0 fail** | ✅ | Clean re-validation. **0 backend failures (~11.3 k req).** Session's commits land off the request hot path: PayMongo ₱50 override (only fires on /payments/create for demo@reelday.ph), dashboard wall-music gate (frontend), upload.html Tala UI gating (frontend), frontend `plans.js` `customMusic` flag add (frontend constant), and wall.html prewarm/kenburns work fully reverted (current wall.html = state before 70d78f0 XSS escapes — pure render-time, not request-path). Numbers reflect that: upload 1000c p99 **3644 ms** in the same range as entry 36's remote best (3417 ms), sustained flat 6/6 at up/rx p95 ~115 ms (clean-baseline territory between entry 32's 85/86 and entry 44's 530/396), multi-wall uploads-poll 168 ms ≈ entry 43's 114 ms record, cross-event Wall A≈B at 207/213 ms (isolation intact). No regression from the day's Tala/music/upload UI work. |
...
### 2026-05-29 — New capacity scenarios: guest-arrival storm + multi-event concurrency

Two new stress scripts added to probe capacity dimensions the existing suite never covered: `stress-arrival.mjs` (the "MC says scan the QR now" simultaneous cold first-load spike, hitting the previously-untested `/api/events/:slug`) and `stress-multi-event.mjs` (N distinct events ALL fully hot at once — aggregate capacity, vs cross-event's 1-hot-1-quiet isolation check).

| # | Test | Cmd | Env | Result | Verdict | Note |
|---|---|---|---|---|---|---|
| 47 | Guest-arrival storm (ramp) | `stress-arrival cxzv-fe0y 100,250,500` | remote | **0 fail all 3 waves** (~3 k req). Time-to-wall p50: 100c **1.18 s**, 250c 1.85 s, 500c 7.57 s. p95: 2.97 / 2.51 / 7.96 s | ✅ | **New test + new endpoint coverage** (`/api/events/:slug` had zero stress coverage). Headline: backend served **500 simultaneous cold first-loads (1 000 req in one tick), 0 failures**. Felt-latency *tail* is single-laptop pipe-contaminated — the tell: 250-wave drained in **112 s** but 500-wave in **26 s** (impossible if backend-bound ⇒ a client socket stalled ~110 s in the 250 wave). 0-fail is the trustworthy signal; latency needs a `local` re-run. Also surfaced: `/api/events/:slug` does **2 uncached DB queries/call** ([events.js:520](../backend/routes/events.js#L520)) — never got the wall-poll path's 10 s cache. |
| 48 | Multi-event — 2 hot events | `stress-multi-event cxzv-fe0y,scarlette-skye-td9n 40 30` | remote | **0 server err** · wall p95 1300/1339 ms · rx p95 832/863 ms · up 44 ok (scarlette archived → 48× correct 403) | ✅ | **New test.** Both events fully hot simultaneously held at the same p95 either runs solo. 50-conn pool unbothered. (First run's verdict heuristic falsely flagged the archived-event 403s as failures — fixed: 4xx now classified as policy rejects, not capacity.) |
| 49 | Multi-event — 3 hot events | `stress-multi-event cxzv-fe0y,andrea-jm,scarlette-skye-td9n 40 30` | remote | **0 server 5xx** · 3 transport blips (0.2 %) · wall p95 1014–1407 ms (all 3 events) · rx p95 ~1.0–1.1 s | ✅ | Going 2→3 hot events moved the backend **not at all** — wall p95 identical. The 3 `wall:err` were transport-level (status null), 0 5xx = the documented single-laptop socket-abort artifact. (`andrea-jm` is a real live event — tested with explicit user authorization. Verdict heuristic then split to count only 5xx as a hard ceiling, transport errors only escalate >1 %.) |
| 50 | Multi-event — **4 hot events** | `stress-multi-event cxzv-fe0y,test-payment,andrea-jm,scarlette-skye-td9n 40 30` | remote | **0 server 5xx · transport err 4/2093 (0.19 %) · worst wall p95 959 ms (<1 s)** · 160 guests · 47× correct 403 (archived) | ✅ | **Cleanest multi-event run + new scenario best.** 4 fully-hot concurrent events, wall p95 **959 ms — lower than the 2-event run** (network quieter), proving latency is the remote network floor, not the backend. 50-conn pool nowhere near saturation; isolation intact across 4 events. The two 15001 ms `up` tails = the new 15 s request-timeout firing on wedged laptop sockets (counted as `neterr`, no longer hanging the report). |

**Capacity read-out:** "how many simultaneous live events?" is **not a per-event limit** — events are rows; the box bounds on aggregate concurrent work: **50-conn Postgres pool** ([database.js:361](../backend/plugins/database.js#L361)), **1200 HTTP conns** ([server.js:97](../backend/server.js#L97)), and **10-concurrent Lambda transcode** (the real ceiling for simultaneous *video* uploads). Wall polls are 10 s-cached (≈1 DB query/10 s/event regardless of viewers), so reads are nearly free. **4 fully-hot events held with 0 backend strain; the true ceiling is far higher.** All runs `remote` single-laptop — absolute latencies carry the documented Cloudflare floor + laptop-pipe tail; a `local` (Easypanel-terminal) re-run is the backend-truth follow-up.
...
| 2026-05-24 | 2 | ~24,500 | 0 | perf:full post-SDE-Lambda-migration (entries 44–45). E44 remote elevated; E45 backend-truth local conclusively diagnosed E44 as laptop network pressure. **E45 set 6 new baselines in one run** — first sub-3s 1000c upload (p99 2330 ms), sustained 3-min at 5-7 ms p95 (~14× faster than the previous "new baseline"), multi-wall + cross-event isolation at 30-33 ms. Validates all SDE Lambda migration code is off-hot-path. **Healthiest run on record.** |
| 2026-05-25 | 1 | ~11,300 | 0 | perf:full re-validation (entry 46). Day's changes (Tala UI gating, frontend customMusic flag, PayMongo override for demo email, wall.html prewarm/kenburns reverted) all off the request hot path. Sustained flat 6/6, cross-event isolation held, upload 1000c p99 3644 ms in remote-best range. No regression. |
| 2026-05-29 | 4 | ~10,300 | 0 backend (8 single-laptop transport blips ≤0.2%) | New capacity scenarios (entries 47–50). Guest-arrival storm: **500 simultaneous cold first-loads, 0 fail**, + first `/api/events/:slug` coverage. Multi-event concurrency ramp 2→3→4 hot events: **4 events / 160 guests / 2093 req, 0 server 5xx, wall p95 959 ms** — adding events doesn't move the backend; 50-conn pool nowhere near saturation. Two new scripts (`stress-arrival`, `stress-multi-event`). All remote single-laptop. |
| **Total** | **79** | **~203,400** | **0 backend failures** | |
...
| 2026-05-29 | Added stress-arrival (simultaneous cold first-load, covers `/api/events/:slug`) + stress-multi-event (N hot events at once) | Coverage expanded to the two untested capacity dimensions: simultaneous *arrival* (vs queue-drain) and *aggregate* multi-event load (vs cross-event isolation). Validated 500 simultaneous arrivals + 4 concurrent hot events, 0 backend failures. |
...
| 2026-05-24 | SDE Lambda migration (sdeRenderInvokeLambda + sdePreflight + sde-lambda-poller) + videoTranscode compressed_key persist + admin.ics column drop + photo-message batch fallback | **Zero hot-path regression — confirmed and improved.** Entry 45 `perf:full-local` set **6 new baselines in one run**: upload 1000c p99 **2330 ms** (first sub-3s, beats entry 36's 3417 by 32 %), sustained 3-min up/rx p95 **5-7 ms** (~14× faster than entry 32's 85/86 "new baseline"), multi-wall uploads-poll **33 ms** (3.5× entry 43's 114), cross-event Wall B **30 ms** (5.5× entry 30's 168), mixed reaction p95 **7 ms** (≈ entry 35's 11). All Lambda code paths are off-hot-path by design — poller's DB query is essentially free when no Lambda renders are in flight (which is always while the feature stays beta-gated), sdePreflight only fires on Generate Reel clicks, dispatcher is a single env-var branch. Healthiest run on record. |
